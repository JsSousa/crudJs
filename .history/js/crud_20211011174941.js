    ...
    refreshData = () => {
        ...
        listaProductos = listaProductos.map((obj, i) => `
            <tr>
                ...
                <button type="button" id="btnDelete${i}" name="btn-delete" class="btn btn-danger btn-sm"><i class="fa fa-trash text-ligth" aria-hidden="true" return false></i></button>
                </td>
            </tr>`)
...
const processClick = (element) => {
    ...
    if (element.target.name == listButtons[1] || element.target.parentElement.name == listButtons[1]) { // se valida btn-edit
        ...
        let hideButtons = (tdColActions, listButtons, 'block')
    ...
    if (['btnDelete'].includes(element.target.name)) {
        const reload = document.getElementById('btnDelete');
...
<button type="button" class="btn btn-danger" name="reload" "id="btnDelete${productId}">Cancelar</button>`
