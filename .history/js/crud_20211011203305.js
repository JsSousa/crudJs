...
const processClick = (element) => {
    ...
    if (element.target.name == listButtons[1] || element.target.parentElement.name == listButtons[1]) { // se valida btn-edit
        ...
        childrenArray.filter((item, i) => i >= (min) && i <= (childrenArray.length - 2)).map((item) => item.innerHTML = `<input value="${item.textContent}">`)
    }

    if (element.target.name == 'btnConfirm') {
        console.log("Element Target=>", element.target.parentElement.parentElement.children);
...
<button onClick="window.location.reload();" type="button" class="btn btn-danger" name="btnCancel" id="btnCancel${productId}">Cancelar</button>`
